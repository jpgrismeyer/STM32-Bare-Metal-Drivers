...
This application validates interrupt-driven USART reception using a driver-level IRQ handler and an application callback.

The PC sends commands through the ST-LINK Virtual COM Port. Each received byte triggers the USART1 interrupt, the application ISR calls the USART driver IRQ handler, and the driver notifies the application through `USART_ApplicationEventCallback()`. The callback stores the received byte in a ring buffer, while the main loop processes complete command lines.
...
- store incoming bytes in an application ring buffer
